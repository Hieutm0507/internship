...
class UpPostActivity : AppCompatActivity() {
    ...
    private fun eventListeners() {
        ...
        binding.btPost.setOnClickListener {
            ...
            if (imageUris.isNotEmpty()) {
                Log.d("DEBUG", "Sending post: userId = ${LoginActivity.currentUserId}, content = $content, images count = ${imageFiles.size}")
    ...
    private fun setObserver(imageFiles : List<File>, content : String) {
        postViewModel.addPost(LoginActivity.currentUserId, imageFiles, content)

        postViewModel.addLiveData.observe(this) { state ->
            when {
        ...
        for (uri in uris) {
            try {
                ...
                if (inputStream != null) {
                    ...
                    Log.e("DEBUG", "Cannot open InputStream for URI: $uri")
                }
            } catch (e: Exception) {
                e.printStackTrace()
            }
